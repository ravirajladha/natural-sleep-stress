<?php
 ...
 $sleep = $data['sleep'];
 $sleep_val = $_SESSION['sleep'.$data['test']];
?>
...
<img src="<?php echo URLROOT; ?>/assets/natural_logo.png" alt="" width="100" style="padding:20px;float:right"> <br><br><br>
    <main>
        <form style="margin-top: -450px;" method="POST" id="sleep_test" action="<?php echo URLROOT; ?>/test/sleep_submit/<?php echo $data['test']; ?>">
        <h2 id="question"> <?php echo $sleep->question; ?>
        <br><span style="font-size:18px !important">(<?php echo $data['test']; ?>/5)</span></h2>
            <div class=range data-labels='["1"
                ...
                <input name=range type=range min=0 step=1 max=100 value=<?php echo $sleep_val; ?> >
                <div class=range-output>
                    <output class=output name=output for=range><span></span></output>
                </div>
                <div class=label-container>
                    <div class=left-label><?php echo $sleep->option1; ?></div>
                    <div class=right-label><?php echo $sleep->option4; ?></div>
                </div>
            </div>
        </form>
    </main>
...
<div class="wrapper">
    <?php if($data['test']>1): ?>
    <a href="<?php echo URLROOT; ?>/test/sleep/<?php echo $prev_test; ?>" style="padding-right:100px"><button class="button button2">Previous</button></a>
    <?php endif; ?>
    
   <button class="button" onclick="document.getElementById('sleep_test').submit();">Next</button></a>  
</div>
</div>
